...
TEEHR_EVALUATION_DIR="" # Path to the directory where TEEHR evaluation output will be stored.
...
# Check and read teehr evaluation dir from config file
check_and_read_teehr_eval_config() {
    if [ -f "$TEEHR_EVAL_CONFIG_FILE" ]; then
        LAST_EVAL_PATH=$(cat "$TEEHR_EVAL_CONFIG_FILE")
        echo -e "${INFO_MARK} Last used TEEHR evaluation directory: ${BBlue}$LAST_EVAL_PATH${Color_Off}"
        read -erp "$(echo -e "  ${ARROW} Use this path? [Y/n]: ")" use_last_eval_path

        if [[ -z "$use_last_eval_path" || "$use_last_eval_path" =~ ^[Yy] ]]; then
            TEEHR_EVALUATION_DIR="$LAST_EVAL_PATH"
            echo -e "  ${CHECK_MARK} ${BGreen}Using previously configured evaluation path${Color_Off}"
        else
            echo -ne "  ${ARROW} Enter your TEEHR evaluation output directory path: "
            read -e TEEHR_EVALUATION_DIR
            echo "$TEEHR_EVALUATION_DIR" > "$TEEHR_EVAL_CONFIG_FILE"
            echo -e "  ${CHECK_MARK} ${BGreen}Path saved for future use${Color_Off}"
        fi
    else
        echo -e "${INFO_MARK} ${BYellow}No previous TEEHR evaluation directory configured${Color_Off}"
        echo -e "  ${INFO_MARK} Default: ${BCyan}$DATA_FOLDER_PATH${Color_Off}"
        read -erp "$(echo -ne "  ${ARROW} Enter your TEEHR evaluation output directory path [default: $DATA_FOLDER_PATH]: ")" TEEHR_EVALUATION_DIR
        if [[ -z "$TEEHR_EVALUATION_DIR" ]]; then
            TEEHR_EVALUATION_DIR="$DATA_FOLDER_PATH"
        fi
        echo "$TEEHR_EVALUATION_DIR" > "$TEEHR_EVAL_CONFIG_FILE"
        echo -e "  ${CHECK_MARK} ${BGreen}Path saved for future use${Color_Off}"
    fi
...
# Handle teehr evaluation dir from arguments or config
handle_teehr_eval_path() {
    if [[ -z "$TEEHR_EVALUATION_DIR" ]]; then
        check_and_read_teehr_eval_config
    fi
    mkdir -p "$TEEHR_EVALUATION_DIR" || handle_error "Failed to create TEEHR evaluation directory: $TEEHR_EVALUATION_DIR"
}
...
print_usage() {
    ...
    echo -e "${BCyan}  -e [path]:${Color_Off} Designates the provided path as the TEEHR evaluation output directory."
...
while getopts 'd:e:hrt:y' flag; do
    case "${flag}" in
        d) DATA_FOLDER_PATH="${OPTARG}" ;;
        e) TEEHR_EVALUATION_DIR="${OPTARG}" ;;
        h) print_usage
           exit 1 ;;
...
# Resolve TEEHR evaluation output directory
handle_teehr_eval_path
